...
class RoomInviteDialogModel extends ChangeNotifier{
  final Widget Function(BuildContext, String, RoomInviteDialogModel) getDialog;
  final RoomInviteWsService _roomInviteWsService = locator<RoomInviteWsService>();
  final Socket socket;
  
  String roomId;
  RemoteData<String, RoomInviteResponse> _roomInviteData = RemoteData.notAsked();
  ...
  init(BuildContext context){
    gameInviteSub = _roomInviteWsService.onRoomInvite.listen((response) {
      ...
      showDialog(context: context, builder: (context) => getDialog(context, response.name, this));
    });
  }
